...
public class AthenaSearchServiceImpl implements AthenaSearchService {
    ...
    public SearchResultVo search(SearchParamVo param) {
        ...
        try {
            //3、执行检索请求
            SearchResponse response = restHighLevelClient.search(searchRequest, AthenaElasticSearchConfig.COMMON_OPTIONS);
            //4、分析响应数据，封装成我们需要的格式
    ...
    private SearchResultVo buildSearchResult(SearchResponse response, SearchParamVo param) {
        ...
        //5、2分页信息-总页码-计算
        int totalPages = (int) total % ElasticsearchConstant.PRODUCT_PAGESIZE == 0 ?
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResultVo.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                Result res = productFeignService.attrInfo(Long.parseLong(s[0]));
                if (res.getCode() == 0) {
                    AttrResponseVo data = res.getData("attr", new TypeReference<AttrResponseVo>() {});
                ...
                navVo.setLink("http://search.athena.com/list.html?" + replace);
                return navVo;
    ...
    private SearchRequest buildSearchRequest(SearchParamVo param) {
        ...
        //分页
        searchSourceBuilder.from((param.getPageNum() - 1) * ElasticsearchConstant.PRODUCT_PAGESIZE);
